...
public final class SVGAParser: NSObject {
    ...
    public var enabledMemoryCache: Bool = false
    public var disableLoadFromMemory: Bool = false
    ...
    public func videoCacheKey(_ urlStr: String) -> String {
    ...
    private func parse(withURLRequest urlRequest: URLRequest,
        ...
        // Save failure/completion callbacks (thread-safe)
        let shouldDownload: Bool = Self.mapLock.withLock {
            if let failureBlock {
                Self.failureMap[key, default: []].append(failureBlock)
            }
            let isFirst = Self.completionMap[key] == nil
            Self.completionMap[key, default: []].append(completionBlock)
            return isFirst
        }
    ...
    private func innerParseWithZipData(_ data: Data?,
        ...
        Self.unzipQueue.addOperation { [weak self] in
            ...
            SSZipArchive.unzipFile(atPath: tmpPath, toDestination: cacheDir, progressHandler: { _, _, _, _ in
                // ignore progress
            }, completionHandler: { [tmpPath] path, succeeded, error in
    ...
    private func executeCacheCompletion(_ cacheKey: String, videoItem: SVGAVideoEntity?) {
        let (completions, _): ([SVGACompletionBlock]?, [SVGAFailureBlock]?) = Self.mapLock.withLock {
            let c = Self.completionMap.removeValue(forKey: cacheKey)
            let f = Self.failureMap.removeValue(forKey: cacheKey)
            return (c, f)
        }
        if let list = completions {
            for block in list {
                OperationQueue.main.addOperation {
                    block(videoItem)
                }
    ...
    private func executeCacheFailure(_ cacheKey: String, error: NSError?) {
        let (_, failures): ([SVGACompletionBlock]?, [SVGAFailureBlock]?) = Self.mapLock.withLock {
            let c = Self.completionMap.removeValue(forKey: cacheKey)
            let f = Self.failureMap.removeValue(forKey: cacheKey)
            return (c, f)
        }
        if let list = failures {
            for block in list {
                OperationQueue.main.addOperation {
                    block(error)
                }
    ...
    private func zlibInflate(_ data: Data) -> Data? {
        ...
        return data.withUnsafeBytes { (rawBytes: UnsafeRawBufferPointer) -> Data? in
            var stream = z_stream()
